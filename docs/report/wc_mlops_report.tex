...
the top-4 candidates by cross-validation Poisson negative log-likelihood. A subsequent QA environment backtests promoted 
...
\begin{enumerate}
  ...
  log-likelihood (NLL) as the objective. The top four candidates by CV NLL are selected
  for QA promotion in the MLflow registry.
...
candidates output goal-rate parameters). RPS remains the selection and gating metric
for top-K promotion, QA evaluation, and the champion/challenger gate, where downstream
...
However, model selection (top-K promotion), QA evaluation, and the champion/challenger
gate still use RPS. At these stages the bias is mitigated by the coarseness of the
