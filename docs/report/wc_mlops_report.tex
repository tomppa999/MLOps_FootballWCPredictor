...
\begin{itemize}
  ...
  (Figure~\ref{fig:cluster_k}).  With the original six-column profile (including 
  \texttt{shots\_on\_goal}, \texttt{shots\_off\_goal}, and \texttt{total\_shots} as 
  three separate features), silhouette scores peaked sharply at $k=2$ ($\approx 0.32$) 
  and declined monotonically, suggesting that the redundant shot features dominated the 
  clustering geometry and reduced all structure to a single high-volume versus low-volume 
  axis. After replacing the three collinear shot columns with volume and precision, the
  ...
  $k=4$ ($\approx 0.20$), then declining steadily.  The elbow
  method shows its clearest bend at $k=5$.  We therefore select $k = 4$: it corresponds 
  to the silhouette local maximum ($\approx 0.20$), lies one step before the elbow at 
  $k = 5$, and provides the best balance between cluster cohesion and interpretable 
  granularity without overfitting the geometry to noise.
...
\tbd{NOT UP TO DATE; REPLACE WITH NEW PLOT; REWRITE TWO TACTICAL SUBSECTIONS; EXPLAIN WHY TACTICAL CLUSTERING WAS NOT USED; HOW THE COLUMNS WERE VERY RAW PROXIES; HOW ROLLING AVGS FROM MATCHES AGAINST WEAKER AND STRONGER TEAMS SMOOTHEN OUT}
...
\begin{figure}[h]
  ...
  corner kicks, possession).  The elbow bends most sharply at $k=5$;
  the silhouette curve has a local maximum at $k=4$ ($\approx 0.20$) after a dip
  at $k=3$.  We select $k=4$.}
  \label{fig:cluster_k}
\end{figure}
...
cross-validation RPS. RPS is appropriate for ordered multi-class outcomes (home win / 
draw / away win) derived from score distributions.
...
- Discuss feature importance findings and their alignment with football domain knowledge (Elo differential expected as
\#1; competition tier, rolling form as secondary). 
