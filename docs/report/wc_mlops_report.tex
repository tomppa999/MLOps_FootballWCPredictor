...
largely following the seminal work of Maher~\cite{maher1982modelling}, who
...
that partially accounts for the empirically observed negative correlation between home
...
The pipeline follows a Bronze/Silver/Gold data architecture~\cite{databricks_bigbook},
...
\begin{itemize}
  ...
  \texttt{is\_knockout}, and \texttt{is\_neutral}.
  For 2026 WC group-stage matches where a host nation plays at
  home, \texttt{is\_neutral} is overridden to \texttt{False}.
  An \texttt{is\_cross\_confederation} feature was investigated but removed
  after proving invisible in all model importances and uncomputable at
  inference time without confederation data in tournament pairings.
...
the simulation as deterministic constraints. Additionally, finished WC
results are converted into Gold-compatible rows (with
\texttt{stats\_tier=none}) and appended to the Gold history before
feature computation, so that rolling goal averages for later-round
matches incorporate earlier tournament scores; shot and tactical rolling
features are unaffected because the \texttt{stats\_tier} filter excludes
these rows. It then loads the
...
NUTS (No-U-Turn Sampler) is used for all experimental and production runs; each Optuna
trial draws a single MCMC chain, making the Bayesian candidate the most expensive
model to tune.
...
\begin{description}
  ...
  \item[H3] \textbf{Deep learning models underperform relative to complexity.}
  ...
  \item[H4] \textbf{Bayesian Poisson produces best-calibrated predictions.}
...
The 2022 FIFA World Cup (2022-11-20 to 2022-12-18) is held out as a strict
test set for QA evaluation and is never seen during training or
cross-validation. The holdout mask additionally requires
\texttt{competition\_tier~=~1}, which excludes friendly matches that
happened to fall within the date window, leaving exactly 64 tournament
matches.
...
\begin{enumerate}
  ...
  wall-clock time (the QA phase retrains and evaluates each model on 64
...
\begin{table}[h]
  \centering
  \caption{QA phase --- WC~2022 holdout metrics (64 tournament matches, sorted
  by RPS, lower is better).  Outcome accuracy is the fraction of matches where
  the model's argmax W/D/L prediction is correct.  The champion/challenger gate
  uses holdout RPS as the primary KPI.}
  \label{tab:qa_results}
  \begin{tabular}{clccc}
    \toprule
    Rank & Model & \footnotesize{Holdout RPS} & \footnotesize{Holdout NLL}
         & \footnotesize{Outcome Acc} \\
    \midrule
    1 & Ridge            & \textbf{0.2250} & 3.023 & 50.0\% (32/64) \\
    2 & Random Forest    & 0.2255 & 3.012 & 53.1\% (34/64) \\
    3 & XGBoost          & 0.2263 & 3.011 & 51.6\% (33/64) \\
    4 & Poisson GLM      & 0.2284 & 3.030 & 53.1\% (34/64) \\
    5 & LSTM             & 0.2289 & 3.099 & 54.7\% (35/64) \\
    6 & Bayesian Poisson & 0.2289 & 3.038 & 51.6\% (33/64) \\
    7 & NegBin GLM       & 0.2294 & 3.041 & 53.1\% (34/64) \\
    8 & SARIMAX          & 0.2336 & 3.315 & 50.0\% (32/64) \\
    9 & 1D CNN           & 0.2479 & 3.166 & 42.2\% (27/64) \\
    \bottomrule
  \end{tabular}
\end{table}

Ridge wins with a holdout RPS of 0.2250, despite ranking seventh in CV NLL
(2.8402) --- the most extreme instance of the CV--holdout ranking reversal
discussed in Section~\ref{sec:discussion}.  The CV NLL leader (NegBin GLM,
2.7564) drops to seventh on holdout RPS (0.2294).  Excluding 1D CNN, the
remaining eight models fall within a 0.0046 RPS band (0.2250--0.2296),
making them statistically indistinguishable on 64 matches.

Outcome accuracy paints a complementary picture: LSTM leads at 54.7\%
(35/64), while Ridge --- the RPS champion --- ties for the lowest non-CNN
accuracy at 50.0\% (32/64).  This divergence is expected and informative:
RPS rewards well-calibrated probability spreads and penalises overconfident
incorrect predictions; outcome accuracy rewards correct argmax calls
regardless of calibration.  The two metrics measure different things, and on
a 64-match holdout dominated by a single feature the choice of metric
determines the champion more than the choice of model architecture.  For the
downstream Monte Carlo simulation, which consumes full probability
distributions rather than point predictions, RPS is the appropriate selection
metric.  Ridge was promoted to production champion.

\subsection{Feature Importance}

Table~\ref{tab:importance} summarises the top-3 features by permutation
importance (RPS degradation) for each model.  The \texttt{elo\_diff}
...
\begin{table*}[t]
  ...
  \begin{tabular}{lccc}
    ...
    Model & \#1 & \#2 & \#3 \\
    \midrule
    Random Forest & \texttt{elo\_diff} \scriptsize{+.119}
      & \texttt{roll\_goals\_ag$_h$} \scriptsize{+.001}
      & \texttt{roll\_goals\_for$_h$} \scriptsize{+.001} \\
    ...
    Bayesian Poisson & \texttt{elo\_diff} \scriptsize{+.103}
      & \texttt{days\_since$_h$} \scriptsize{+.001}
      & \texttt{days\_since$_a$} \scriptsize{+.001} \\
    Poisson GLM & \texttt{elo\_diff} \scriptsize{+.103}
      & \texttt{roll\_goals\_ag$_h$} \scriptsize{+.001}
      & \texttt{elo\_sum} \scriptsize{+.001} \\
    LSTM & \texttt{elo\_diff} \scriptsize{+.099}
      & \texttt{is\_neutral} \scriptsize{+.002}
      & \texttt{roll\_goals\_for$_h$} \scriptsize{+.002} \\
    XGBoost & \texttt{elo\_diff} \scriptsize{+.097}
      & \texttt{roll\_goals\_ag$_h$} \scriptsize{+.002}
      & \texttt{roll\_tac\_poss$_a$} \scriptsize{+.002} \\
    Ridge & \texttt{elo\_diff} \scriptsize{+.064}
      & \texttt{roll\_goals\_ag$_h$} \scriptsize{+.003}
      & \texttt{roll\_goals\_ag$_a$} \scriptsize{+.003} \\
    1D CNN & \texttt{elo\_diff} \scriptsize{+.059}
      & \texttt{is\_neutral} \scriptsize{+.004}
      & \texttt{roll\_goals\_ag$_a$} \scriptsize{+.003} \\
    \bottomrule
  \end{tabular}
\end{table*}
...
First, \texttt{elo\_diff} importance is 10--100$\times$ larger than any
secondary feature, confirming that Elo encodes the vast majority of
accessible predictive signal in this dataset; all other features are
marginal corrections to a first-order Elo prediction.
Second, XGBoost is the only model whose top-3 includes a tactical feature
(\texttt{away\_team\_rolling\_tac\_possession\_pct}), which it receives via
its extended 28-feature set; the permutation gain is small ($+0.002$) and
should be read as suggestive rather than conclusive.
Third, the deep learning models (LSTM, CNN) assign notably higher importance
to \texttt{is\_neutral} than any other family, suggesting they compensate
for weaker exploitation of the Elo differential by leaning on the venue
signal.  The Gold-v2 features \texttt{elo\_sum} and \texttt{rest\_diff}
  surface in top-3 importance for Random Forest and Bayesian Poisson
  respectively; the remaining added features do not appear in any model's
  top three.
...
magnitude above any other feature.  \texttt{elo\_sum} surfaces in the top three for Random Forest and SARIMAX,
giving Elo information two distinct top-rank entries in those families.  The
...
Poisson GLM ranks third in CV NLL (2.7605) and fourth in holdout RPS
(0.2285), within 0.004 of the RPS champion and only 1 correct prediction
behind the outcome accuracy leader (34/64 vs.\ 35/64).  It outperforms
Bayesian Poisson, NegBin GLM, SARIMAX, and CNN on both metrics,
confirming that the classical Poisson assumption provides a strong baseline
that more complex architectures fail to substantially improve upon.  This
result is a direct consequence of the Elo-dominated feature ceiling: once
Elo is accounted for, the choice of distributional assumption matters far
less than the quality of the input features.

\textbf{H3: Deep learning underperforms relative to complexity ---
partially confirmed.}
CNN ranks last in both RPS (0.2479) and outcome accuracy (42.2\%, 27/64),
confirming the data-sparsity penalty for deep convolutional architectures
on a national-team corpus.  LSTM tells a more nuanced story: it ranks fifth
in RPS (0.2289) but leads all models in outcome accuracy (54.7\%, 35/64),
suggesting it captures sharp outcome signals that RPS --- which penalises
overconfident incorrect predictions --- partially discounts.  The hypothesis
is confirmed for CNN but not for LSTM, which proves competitive on the
clean 64-match WC-only holdout.  Neither deep learning model, however,
breaks through the Elo-imposed feature ceiling: even LSTM's best-in-class
accuracy lead amounts to only 3 additional correct predictions out of 64,
well within sampling noise.

\textbf{H4: Bayesian Poisson produces best-calibrated predictions --- not
confirmed.}
Bayesian Poisson ranks sixth in holdout RPS (0.2289, tied with LSTM),
behind Ridge, Random Forest, XGBoost, and Poisson GLM.  Its holdout NLL
(3.038) is higher than XGBoost's (3.011) and Poisson GLM's (3.030).
Despite using full MCMC sampling (NUTS), the posterior predictive
distributions do not translate into superior W/D/L calibration on this
holdout set.  A possible explanation is that the single-chain, moderate-draw
budget used during Optuna tuning (500--2000 draws, 500--2000 tune steps)
leaves residual sampling noise in the posterior means, negating the
theoretical calibration advantage of the Bayesian framework.  Bayesian Poisson is, however, the one model that exploits the
squad-cohesion signal (\texttt{rest\_diff} ranks second in its
importance), consistent with prior-driven samplers being more receptive to
weak-but-real signals than maximum-likelihood baselines.  The Bayesian framework's calibration advantage
does not materialise in practice, and its computational cost (~7 hours for
40 NUTS trials) is three orders of magnitude higher than Ridge (seconds,
single hyperparameter), which achieves better holdout RPS.

\textbf{SARIMAX.}
SARIMAX ranks eighth in holdout RPS (0.2336) and last in holdout NLL
(3.315), substantially higher than all other models.  The integer
...
temporal structure (Section~\ref{sec:threats}).  Like the Bayesian and
deep learning candidates, SARIMAX cannot improve on the simpler regularised
linear baseline when the signal is Elo-dominated.

\subsection{Feature Iteration and Holdout Refinement}
\label{sec:gold_v2}

A second pipeline run added four features (\texttt{elo\_sum},
\texttt{rest\_diff}, \texttt{home\_days\_since\_last\_match},
\texttt{away\_days\_since\_last\_match}) and tightened the holdout to
WC-tier matches only (\texttt{competition\_tier~=~1}), reducing it from 89
to 64 matches by excluding ~25 friendly matches that fell within the
WC~2022 date window.  This holdout correction changed model rankings
substantially (Ridge moved from rank~3 to rank~1) and invalidated the
prior champion's stored RPS, so the champion alias was reset and Ridge was
promoted on the authoritative 64-match set.  All numbers in this paper
reflect the cleaned holdout.  Among the new features, \texttt{elo\_sum}
proved the most useful (top-5 importance in four models); the cohesion
proxies only registered for Bayesian Poisson; \texttt{is\_cross\_confederation}
was invisible across all nine models and was subsequently removed
(CORE~12, FULL~28).  The iteration demonstrates the continuous-improvement
loop working as designed: the full pipeline re-ran and the auto-promotion
gate evaluated the new configuration without manual intervention.

\subsection{CV--Holdout Ranking Reversal}

The most notable structural result is the ranking reversal between the
experimental and QA phases.  The top-three CV NLL models (NegBin GLM,
Bayesian Poisson, Poisson GLM) all drop to the lower half of the holdout
RPS ranking, while Ridge --- the seventh-ranked model by CV NLL --- wins
on holdout RPS.  This divergence has three plausible explanations:
...
\begin{enumerate}
  ...
  tournament-only sample of 64 matches with higher average competitive
  ...
  generalise less well to a pure tournament environment.
  \item \textbf{Elo-dominated feature ceiling.}  When a single feature
  (\texttt{elo\_diff}) accounts for the vast majority of accessible signal,
  the residual that model architecture can exploit is small and noisy.  On a
  64-match holdout, sampling variance in that residual dominates, making
  CV-based ranking an unreliable predictor of holdout performance.
\end{enumerate}

A further dimension emerges from the outcome accuracy analysis: the RPS
champion (Ridge, 50.0\% accuracy) and the accuracy leader (LSTM, 54.7\%)
are different models.  This confirms that the metric mismatch extends
beyond CV vs.\ holdout: RPS and accuracy measure different properties of
the same predictions, and on this holdout the choice of evaluation metric
determines the champion more than the choice of model architecture.
...
remained, Ridge (rank~7 in CV NLL but rank~1 in holdout RPS) and Random
Forest (rank~5 in CV NLL but rank~2 in holdout RPS) would have been
excluded from QA entirely, based on a CV metric that proved misleading.

\subsection{Narrow Result Spread}

Excluding CNN, the remaining eight models cluster within a 0.0046 holdout
RPS band (0.2250--0.2296) --- roughly half the spread seen in previous
configurations.  Four structural factors explain this:

\begin{enumerate}
  \item \textbf{Shared feature ceiling.} All core-feature models consume the
  same 12 features, with Elo differential accounting for the vast majority of
  ...
  captures, and that residual is small.
  ...
  limit the achievable prediction accuracy for any model family.  50--55\%
  outcome accuracy is broadly consistent with bookmaker-implied base rates
  once the margin is stripped.
  ...
  \item \textbf{Efficiency implication.} The negligible performance
  differences carry a practical consequence: Ridge tunes in seconds with a
  single hyperparameter ($\alpha$), while Bayesian Poisson requires
  approximately seven hours of NUTS sampling and XGBoost approximately nine
  minutes across 100 Optuna trials.  When the Elo-imposed feature ceiling
  makes architecture effectively irrelevant, computational efficiency becomes
  the rational tiebreaker.  A production system should prefer the simplest
  model that reaches the ceiling --- in this domain, Ridge with Elo
  differential and a handful of carefully chosen secondary features.
\end{enumerate}

The practical implication is that champion selection in this domain is
sensitive to the holdout sample: a different 64-match tournament could
...
partially address this by explicitly encoding the gap between consecutive
national-team matches per side; only Bayesian Poisson exploited the signal
in permutation importance analysis, so the threat persists for the other
eight families.
...
(exactly one group-stage matchday: 12~groups $\times$ 2~matches) is compared against
...
model with fewer than 24 scored matches is silently skipped to suppress
...
swapped in for the fixed 24-match window used here.
...
Ridge was selected as the production champion based on its holdout RPS
(0.2250) on the 2022 FIFA World Cup, despite ranking seventh in
cross-validation NLL (2.8402) --- the most extreme instance of the
CV--holdout ranking reversal that characterises this domain.  This reversal
led to an empirically motivated design change: removing the original
top-four promotion gate in favour of advancing all nine candidates to QA,
ensuring that no model is excluded based on a metric that may not predict
holdout performance.

The central finding is that Elo differential dominates predictive signal so
completely that model architecture becomes secondary.  Permutation importance
places \texttt{elo\_diff} first in all nine models, with importance scores
10--100$\times$ above any secondary feature.  The top eight models cluster
within a 0.0046 holdout RPS band, and outcome accuracy analysis reveals
that the RPS champion (Ridge, 50.0\%) and the accuracy leader (LSTM,
54.7\%) are different models --- underscoring that model selection is
metric-dependent rather than architecture-dependent in this domain.  This
Elo-imposed feature ceiling means that a simple linear model with Elo
differential and a small set of secondary features captures effectively all
accessible signal.  In this regime, computational efficiency is the rational
tiebreaker: Ridge tunes in seconds with a single hyperparameter, while
Bayesian and deep learning candidates require hours for no measurable
improvement.

A Gold-v2 feature expansion (cross-team Elo composition, squad-cohesion
proxies) was carried out to exercise the continuous-improvement workflow
end-to-end, and a concurrent holdout refinement (restricting to WC-tier
matches, 89~$\to$~64 matches) validated the importance of clean holdout
construction.  The auto-promotion gate and full MLflow audit trail
demonstrate the iteration loop functioning as designed.

\textbf{Limitations and future work.}
The holdout evaluation relies on a single tournament (64 matches), which
limits statistical power to distinguish closely ranked models --- the 0.0046
RPS spread among the top eight is within sampling noise on this sample size.
Future work should extend the holdout to include multiple tournaments
(e.g.\ EURO~2024, Copa América~2024) or employ cross-tournament validation
to establish whether the Ridge advantage is a consistent signal or an
artefact of a single tournament.  The efficiency argument for Ridge would
be further strengthened if it holds across multiple holdouts; if it does
not, the feature-ceiling interpretation implies the ranking is arbitrary
rather than meaningful.  Additional modelling improvements include
multi-chain MCMC sampling for the Bayesian model and calendar-aware time
indexing for SARIMAX.
