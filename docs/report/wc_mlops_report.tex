...
\texttt{wc\_production} model, and promoted via the \texttt{champion} alias. XGBoost is
selected as champion despite ranking fourth in cross-validation, illustrating a ranking
reversal between the tuning and holdout metrics that motivated removing the original
top-four promotion gate. A Monte Carlo simulation derives tournament outcome
...
logged to MLflow. The best candidate is promoted via the \texttt{champion} alias in the MLflow
...
was inverted on the WC~2022 holdout, where XGBoost achieved the lowest RPS despite
ranking fourth in CV.  Two observations motivated removing the top-K gate:
\begin{enumerate}
...
models (LSTM, CNN) and SARIMAX occupy the lower ranks.
...
XGBoost is the only model to surface a tactical rolling feature
(\texttt{away\_team\_rolling\_tac\_possession\_pct}) in its top three,
likely because it is the only candidate that receives the full 24-feature
set and can learn non-linear splits on the additional tactical columns.
...
from $+0.060$ (CNN) to $+0.118$ (Random Forest) — one to two orders of
...
the margin over the next-best model (LSTM, 0.2133) is only 0.0015,
limiting the strength of the conclusion.
...
LSTM, however, ranks second (RPS 0.2133), outperforming all statistical
...
Permutation importance analysis confirmed that Elo differential is the
...
The Monte Carlo simulation layer, which derives tournament outcome
probabilities from predicted score distributions, is planned but not yet
integrated.  Additional improvements include full MCMC sampling for the
