...
\begin{itemize}
  ...
  An \texttt{is\_cross\_confederation} feature was investigated in the Gold v2
  iteration but removed after proving invisible in all model importances
...
the full 28-feature set, so the other eight were never offered the
...
specifically in its \emph{away}-side variant.  One plausible reading is
that rolling possession is a \emph{dominance} marker that partially
overlaps with \texttt{elo\_diff} --- consistent with the
volume-and-dominance continuum observed in the clustering experiment
(Figure~\ref{fig:cluster_k}) --- but encodes a residual playstyle axis
that Elo, which scores results rather than the manner in which they are
obtained, does not fully absorb.  The away-side variant in particular may
surface because home possession is confounded with the home-advantage
effect, whereas sustained away possession must be imposed against both the
opponent and the venue, making it a cleaner proxy for proactive style that
XGBoost can combine non-linearly with \texttt{elo\_diff}.  The permutation
gain is small ($+0.002$ $\Delta$ NLL) and should be read as suggestive
...
itself becomes diagnostic evidence for H3 rather than a counterexample
(Section~\ref{sec:gold_v2}).
...
higher than XGBoost's (2.9155) and Poisson GLM's (2.9532).  Despite
using full MCMC sampling (NUTS), the posterior predictive distributions
do not translate into superior W/D/L calibration on this holdout set.
A possible explanation is that the single-chain, moderate-draw budget
used during Optuna tuning (500--2000 draws, 500--2000 tune steps)
leaves residual sampling noise in the posterior means used for
prediction, negating the theoretical calibration advantage of the
Bayesian framework.  Bayesian Poisson is, however, the one model that
exploits the squad-cohesion signal added in the Gold v2 iteration
(\texttt{rest\_diff} ranks second in its importance).  This is
consistent with prior-driven samplers being more receptive to
weak-but-real signals than maximum-likelihood baselines
(Section~\ref{sec:gold_v2}).
...
\texttt{CORE\_FEATURE\_COLUMNS} grew from 8 to 12 (13 before the subsequent
removal of \texttt{is\_cross\_confederation}); \texttt{FULL\_FEATURE\_COLUMNS}
(XGBoost only) from 24 to 28.
...
operating near its capacity ceiling; adding 4 features to a 12-feature
...
in the H3 paragraph above, this regression is itself diagnostic: it
...
\texttt{is\_cross\_confederation} was invisible in all model importances:
...
access to the same signal under a different name.  It was subsequently
removed from the Gold schema (CORE 13\,$\to$\,12, FULL
29\,$\to$\,28), as it also could not be computed at inference time
without confederation data in the tournament pairings.
...
\begin{enumerate}
  ...
  same 12 features, with Elo differential accounting for the majority of
...
Additional modeling improvements include multi-chain MCMC sampling with
higher draw budgets for the Bayesian model and calendar-aware time
