...
rolling averages from any newly completed matches. It then loads the
\texttt{champion}-aliased model from the MLflow Model Registry, predicts
upcoming matches, runs the Monte Carlo tournament simulation, and logs
updated predictions as MLflow artifacts. The model artifacts are frozen;
...
Group-stage matches are resolved by Poisson scoreline sampling from predicted
rates. Knockout-round draws after 90~minutes are resolved by sampling extra
...
tiebreaking. The $n_\mathrm{sims}$ parameter is configurable; 10{,}000 is
sufficient for stable tournament-level advancement probabilities without
excessive runtime.
...
Two patterns stand out beyond the \texttt{elo\_diff} dominance.  
First, XGBoost actually surfaced one of the tactical rolling features,
...
\subsubsection{Simulation Simplifications}
...
(approximately 4--5 matches per team between the initial training run and
