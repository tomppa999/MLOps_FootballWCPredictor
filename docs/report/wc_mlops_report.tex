...
\begin{enumerate}
  ...
  serialized as an MLflow pyfunc artifact, registered as \texttt{wc\_production}, and used for tournament simulation.
\end{enumerate}
...
Optuna would both invalidate that contract and add roughly 75 minutes to
every cycle. Each refit is logged as a new MLflow run tagged
...
This is partly structural --- XGBoost is the only model that receives
the full 24-feature set, so the other eight were never offered the
tactical columns to rank --- but the feature is still chosen over the
six other tactical columns that XGBoost did have access to, and
specifically in its \emph{away}-side variant.  The permutation gain is
small ($+0.001$ $\Delta$ RPS) and should be read as suggestive rather
...
generalisation.  It is also the only model whose top-three importance
contains a tactical feature
(\texttt{away\_team\_rolling\_tac\_possession\_pct}), enabled by its access
to the full 24-feature set.  One plausible reading is that rolling
...
Importantly, the permutation importance is only $+0.001$ $\Delta$ RPS,
and the holdout margin over the next-best model (LSTM, 0.2133) is only
0.0015, so this interpretation is offered as a hypothesis rather than a
confirmed mechanism.
...
any alert condition --- that would violate the project's code-based-
