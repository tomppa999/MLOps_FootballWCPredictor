...
promotion, and cloud scheduling.
...
from matches with statistics coverage, Elo difference and sum,
squad-cohesion proxies, and context flags on tournament and match level are
...
$\hat{\lambda}_h$ and $\hat{\lambda}a$. Tournament advancement probabilities 
are then obtained by Monte Carlo simulation ($n\mathrm{sims}=10{,}000$ 
paths), sampling scorelines for every of the $\binom{48}{2} = 1{,}128$ 
...
\textbf{Tuning} minimises Poisson NLL (goal-rate fidelity); \textbf{QA 
gating} uses holdout RPS because simulation consumes outcome probabilities. 
The two metrics are deliberately decoupled: NLL rewards placing probability 
mass near the actual scoreline and is agnostic to outcome ordering, making it 
a stable signal for hyperparameter search. RPS evaluates the cumulative W/D/L 
distribution and directly measures what matters for tournament advancement 
prediction. Using RPS as the tuning objective would create a draw-hedging 
incentive, since draw occupies the middle ordinal position and a confident 
incorrect draw prediction never incurs the maximum penalty.
All nine candidates advance to QA after an initial top-4 gate proved 
misleading when CV and holdout rankings diverged (Section~\ref{sec:impl_results}).
...
Scheduler entries. Daily runs are sufficient to capture the last friendly matches 
of qualified teams before the tournament starts, and a 30-minute run interval during the World Cup makes sure that predictions stay up-to-date.
...
This CV--holdout ranking reversal reflects three factors: (i)~NLL rewards
scoreline-level calibration while RPS collapses predictions into three W/D/L
buckets, so the two metrics can rank the same models differently;
(ii)~the WC~2022 holdout is a tournament-only sample with stronger teams and
more neutral venues than the training average, introducing distribution shift
from the overall training data;
(iii)~when \texttt{elo\_diff} accounts for the vast majority of signal, the
residual that architecture can exploit is small and noisy on 64~matches.

Excluding CNN, eight models lie within a 0.0046 RPS band (0.2250--0.2296),
making them statistically indistinguishable.
Outcome accuracy tells a complementary story: LSTM leads at 54.7\% (35/64)
while Ridge (the RPS champion) achieves only 50.0\% (32/64).
RPS rewards well-calibrated probability spreads; accuracy rewards correct
argmax calls regardless of confidence.
On this holdout the choice of metric determines the champion more than the
...
This reversal also motivated a pipeline design change: the original top-four
CV promotion gate was removed in favour of advancing all nine candidates to
QA, since the CV metric proved misleading.
...
Permutation importance (Appendix~\ref{app:model}) ranks Elo difference first
in all nine models with degradation scores 10--100$\times$ larger than any
secondary feature (rolling goals against, competition tier).
...
QA uses 64 matches from one World Cup. As mentioned, the 0.0046 RPS spread among 
top models is within sampling noise, so the champion choice may not generalise.

\textbf{Sparse statistics.}
Only $\sim$51\% of matches have per-match stats. Coverage depends heavily on the 
confederation; it is lower for CAF, AFC, and OFC, so tactical features are likely 
biased toward UEFA/CONMEBOL-heavy training data.
