...
\begin{figure*}[t]
  \centering
  \maybegraphic{../figures/Architecture_final.png}{0.95\textwidth}
...
retraining as new data arrives~\cite{google_mlops}.
...
Poisson components (team-1-specific $\lambda_1$, team-2-specific $\lambda_2$, and
...
Moreschini et al.~\cite{moreschini2022mlops} propose a graphical representation 
of the MLOps pipeline as an extension of DevOps for ML-based software systems and 
discuss research challenges related to automation maturity and tooling.
...
We target Google MLOps Level~1~\cite{google_mlops}: DVC orchestrates
...
DVC versions raw snapshots, Silver Parquet, and Gold artifacts;
MLflow tracks experiments and manages registry promotion;
Git and pytest provide the DevOps base.
DagsHub is used as a unified remote for data, code and model artifacts.

We adopt code-based deployment~\cite{databricks_bigbook}: the three
...
Bronze stores immutable API-Football JSON and Elo TSV snapshots.
Silver is schema-validated, season-partitioned Parquet, joining data 
from both sources and adding context, such as competition tiers or knockout 
flags.
Finally, Gold consists of one row per match with strictly time-aware features 
(rolling form, Elo composites, context flags); no future information enters 
any column.
...
\begin{figure*}[t]
  \centering
  \maybegraphic{../figures/cluster_experiment_v2.png}{0.95\textwidth}
...
Tuning minimises Poisson Negative log-likelihood (NLL); QA gating uses holdout 
Ranked Probability Score (RPS).
The two metrics are deliberately decoupled.

Poisson NLL rewards placing probability mass near the actual
scoreline:
\begin{equation}
...
RPS evaluates the cumulative W/D/L distribution:
\begin{equation}
...
Figure~\ref{fig:process} summarises the end-to-end process from ingestion
trigger through training, deployment, and live monitoring.

\begin{figure*}[t]
  \centering
  \maybegraphic{../figures/Process.png}{0.95\textwidth}
  \caption{End-to-end process: ingestion trigger, continuous training,
  deployment, and live monitoring workflow.}
  \label{fig:process}
\end{figure*}
...
\textbf{Source desynchronisation.}
Because the two external sources update independently, transient
desynchronisation is possible (e.g.\ API-Football settles a fixture before
eloratings.net incorporates the result, or vice versa).
The pipeline mitigates this by design: \texttt{merge\_asof} always joins the
most recent Elo observation \emph{before} match date, so an early Elo update
does not leak post-match information.
If fixtures settle before Elo incorporates the result, pre-match Elo is still
the latest available, which is the intended feature semantics.
However, no explicit consistency check verifies that both sources agree on
the set of completed matches.
...
Future work could use the best-third-place rule as a natural setting to study cross-group dependency in the expanded World Cup format, where a result in one group can affect advancement probabilities in another. In particular, the 30-minute inference cycle during WC 2026 provides an opportunity to analyse how sequential group-stage result revelation reshapes qualification probabilities over time. By storing simulation snapshots after each completed match, future analyses could trace per-team probability trajectories across matchdays, identify structural drivers of disproportionate probability swings (e.g., proximity to the third-place boundary and the number of groups already completed) and quantify the gradual resolution of tournament uncertainty using Shannon entropy over the advancement probability distribution.

Further, champion models should be validated across multiple tournaments (e.g.\
